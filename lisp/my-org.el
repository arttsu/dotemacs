...
PLAN should be either \"A\" or \"B\"."
  (let ((file (expand-file-name "local/gym/gym-0.org" my-org-dir))
...
(defun my-org-capture-templates ()
  ...
  `(("i" "Inbox")
    ...
    ("j" "Journal")
    ("jj" "local entry" entry (file+olp+datetree ,(expand-file-name "local/journals/journal-0.org" my-org-dir)) "%<%H:%M> %?")))
...
(defun my-org-format-buffer ()
  ...
  (interactive)
  (org-map-entries (lambda () (ignore-errors (my-org-sort-entries))) "sort" 'file)
