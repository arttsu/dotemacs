...
(when (executable-find "ledger")
  (use-package ledger-mode
    ...
    (ledger-reports-add "bal-last-week" "%(binary) -f %(ledger-file) --invert --period 'last week' -S amount --limit 'account !~ /:(MM|YY)$/' bal ^Income ^Expenses")
    (ledger-reports-add "fixed-last-month" "%(binary) -f %(ledger-file) --invert --period 'last month' bal MM YY")))
